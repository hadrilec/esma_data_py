...
def get_last_full_files(isin=None, cfi=None, eqt=True):
    ...
    mifid_file_list[["filetype", "date", "cfi", "nfile"]] = mifid_file_list[
        "file_name"
    ].str.split("_", expand=True, n=3)

    list_col = ["instrument_type", "file_type", "filetype", "cfi"]
    mifid_file_list2 = mifid_file_list[list_col + ["file_name"]].drop_duplicates(
        subset=list_col
    )

    mifid_file_list = mifid_file_list[
        mifid_file_list["date"] == max(mifid_file_list["date"])
    ]

    if eqt:
        mifid_file_list = mifid_file_list[
            mifid_file_list["instrument_type"] == "Equity Instruments"
        ]
    else:
        mifid_file_list = mifid_file_list[
            mifid_file_list["instrument_type"] == "Non-Equity Instruments"
        ]
